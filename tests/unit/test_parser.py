...
def test_generate_parquet_urls_default_datasets():
    ...
    # Should contain 4 urls (yellow, green, manhattan, fhv)
    assert len(urls) == 4
    datasets_found = {url.split("_")[0] for url in urls}
    assert datasets_found == {"yellow", "green", "manhattan", "fhv"}
...
def test_generate_parquet_urls_invalid_types():
    ...
    with pytest.raises(TypeError, match="must all be iterable"):
        generate_parquet_urls(datasets=None, years=None, months=None) # Wait, datasets=None is OK, years/months are required
