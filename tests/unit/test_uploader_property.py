...
def test_returns_tuple_with_path_and_str(source_path, bucket_path):
...
@given(source_path=st.text(), bucket_path=st.text())
def test_file_name_from_bucket_path(source_path, bucket_path):
...
@given(source_path=st.text(), bucket_path=st.text())
def test_source_path_preserved(source_path, bucket_path):
...
def test_file_name_no_directory(source_path, bucket_path):
    _, file_name = _extract_file_info(source_path, bucket_path)
...
def test_file_name_nested_path(source_path, segments):
...
def test_bucket_path_trailing_slash(source_path, bucket_path):
    _, file_name = _extract_file_info(source_path, bucket_path)
...
def test_status_is_enum(status):
...
def test_success_skipped_have_hash(
    file_name, original_path, status, hash_value, hash_type
):
    result = UploadResult(
        file_name, original_path, status, hash_value=hash_value, hash_type=hash_type
    )
...
def test_error_has_message(file_name, original_path, error_message):
    result = UploadResult(
        file_name, original_path, UploadStatus.ERROR, error_message=error_message
    )
...
def test_success_has_no_error_message(file_name, original_path, error_message):
    result = UploadResult(
        file_name, original_path, UploadStatus.SUCCESS, error_message=error_message
    )
...
def test_default_error_message_empty():
    result = UploadResult("f.txt", "/path/f.txt", UploadStatus.SUCCESS)
...
def test_defaults_not_none(file_name, original_path):
